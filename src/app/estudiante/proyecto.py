...
cursor = conn.cursor()
class Proyectos():
    def __init__(self, nombre_proyecto, ciclo, estudiante_uno, estudiante_dos, estudiante_tres, jurado_ciclo, fecha_hora_entrega, hash_proyecto, hash_anexos, estado, entrega):
    ...
    def set_proyecto(self):
        ...
        sql="INSERT INTO projecto_proyecto (nombre_proyecto, ciclo, fecha_hora_entrega, id_hash_documento, id_hash_anexos, estudiante_dos_id, estudiante_tres_id, estudiante_uno_id, jurados_ciclo_id, estado_calificado, entrega) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"
        cursor.execute(sql, (self.nombre, self.ciclo, self.fecha_hora_entrega, self.hash_proyecto, self.hash_anexos, self.estudiante_uno, self.estudiante_dos, self.estudiante_tres, self.jurado_ciclo, self.estado, self.entrega))
        conn.commit()
        if cursor.rowcount == 1:
            ...
            sql_estado="INSERT INTO projecto_entrega (enviado, id_proyecto) VALUES (%s, %s)"
            cursor.execute(sql_estado, (self.estado, proyecto[0]))
            conn.commit()
